...
Item {
    ...
    function getIconCode(iconName) {
        const resolvedName = Utils.resolveIconName(iconName)
        var code = FontIconLoader.fontIconIndex[resolvedName]
    ...
    Text {
        ...
        text: isFontIcon
            ? (isUnicode ? icon : (root.getIconCode(icon) > 0 ? String.fromCharCode(root.getIconCode(icon)) : "□"))
            : ""
        font.family: FontIconLoader.fontFamily
        font.pixelSize: size
        visible: isFontIcon
    }
