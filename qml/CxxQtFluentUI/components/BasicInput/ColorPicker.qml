...
ColumnLayout {
    ...
    property string colorName: getColorName(hue, saturation, value)
    ...
    // Optional sections (match upstream API)
    ...
    property color controlSolidColor: ThemeManager.currentTheme && ThemeManager.currentTheme.colors ?
        ThemeManager.currentTheme.colors.controlSolidColor : "#ffffff"
        ...
        Item {
            ...
            layer.effect: OpacityMask {
                maskSource: Rectangle {
                    width: colorField.width
                    height: colorField.height
                    radius: buttonRadius
                }
            }
            ...
            Rectangle {
                ...
                color: "transparent"
                x: root.ringMode ?
                    colorField.width / 2 + root.saturation * Math.min(colorField.width, colorField.height) / 2 * Math.cos(root.hue * Math.PI / 180) - 7 :
                    (root.hue / 360) * colorField.width - 7
                y: root.ringMode ?
                    colorField.height / 2 + root.saturation * Math.min(colorField.width, colorField.height) / 2 * Math.sin(root.hue * Math.PI / 180) - 7 :
                    (1 - root.saturation) * colorField.height - 7
            ...
            MouseArea {
                ...
                function updateFromMouse(mx, my) {
                    var w = colorField.width
                    var h = colorField.height
                    if (root.ringMode) {
                        var dx = mx - w / 2
                        var dy = my - h / 2
    ...
    ColumnLayout {
        ...
        Slider {
            ...
            handle: Rectangle {
                ...
                color: textColor
                border.width: 4
                border.color: controlSolidColor
            }
        }
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            Rin.ToolButton {
                ...
                iconName: collapsed ? "ic_fluent_chevron_down_20_regular" : "ic_fluent_chevron_up_20_regular"
        ...
        RowLayout {
            visible: root.moreVisible && !collapsed
            Layout.fillWidth: true
            spacing: 16
        ...
        ColumnLayout {
            visible: root.moreVisible && !collapsed && (root.colorChannelInputVisible || (root.alphaEnabled && root.alphaInputVisible))
            Layout.fillWidth: true
            spacing: 12

            Repeater {
                model: root.colorChannelInputVisible && channelMode.displayText === "RGB"
                    ? [{ label: qsTr("Red"), m: "r" }, { label: qsTr("Green"), m: "g" }, { label: qsTr("Blue"), m: "b" }]
                    : (root.colorChannelInputVisible
                ...
                RowLayout {
                    ...
                    Rin.TextField {
                        ...
                        text: activeFocus ? text : (channelMode.displayText === "RGB"
                        ...
                        onTextEdited: {
                            ...
                            if (isNaN(val)) return
                            if (channelMode.displayText === "RGB") {
    ...
    function getColorName(h, s, v) {
        ...
        if (s < 0.08) return v > 0.9 ? "White" : "Gray"
        if (v > 0.8 && s < 0.45) {
