...
Item {
    ...
    // When set, pages receive this object as `navigationView` instead of the sidebar itself.
    // This matches the gallery pattern (window.navigationView.push()) while keeping
    // backward compatibility for existing pages.
    property var navigationViewRoot: null
    ...
    function buildPageUrl(cleanPage) {
        // Prefer direct path (supports subdirectories), but also support
        // legacy keys like "unexist/page" by falling back to a flat filename.
        var directUrl = Qt.resolvedUrl("../" + pagesDirectory + "/" + cleanPage + ".qml")
        if (String(cleanPage).indexOf("/") === -1) return directUrl
        var flatName = String(cleanPage).replace(/\\//g, "_")
        var flatUrl = Qt.resolvedUrl("../" + pagesDirectory + "/" + flatName + ".qml")
        return [directUrl, flatUrl]
    }
    ...
    function navigate(page, properties, addToHistory) {
        ...
        if (stackView) {
            ...
            var urlCandidate = buildPageUrl(cleanPage)
            var component
            if (urlCandidate instanceof Array) {
                component = Qt.createComponent(urlCandidate[0])
                if (component.status !== Component.Ready) {
                    component = Qt.createComponent(urlCandidate[1])
                }
            } else {
                component = Qt.createComponent(urlCandidate)
            }
            if (component.status === Component.Ready) {
                if (properties.navigationView === undefined) {
                    properties.navigationView = navigationBar.navigationViewRoot || navigationBar
                }
                ...
                var errorInstance = errorComponent.createObject(stackView, {
                    ...
                    navigationView: navigationBar.navigationViewRoot || navigationBar
                })
                if (!errorInstance) {
