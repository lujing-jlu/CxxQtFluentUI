...
// NOTE: This file is exported as `Rin.FloatLayer` via `components/qmldir`.
// Keep it self-contained (like Rin-UI) so pages can call `floatLayer.createInfoBar(...)`
...
Item {
    ...
    function createInfoBar(options) {
        var component = infoBarComponent || defaultInfoBarComponent
        try {
            ...
            var infoContainer = component.createObject(determinePosition(position), {
                title: title,
                text: text,
                severity: severity,
                position: position,
    ...
    function createCustom(component) {
        try {
            ...
            if (!infoContainer)
                throw "Failed to create component instance."
